...
from App.controllers import get_allocations, allocate_staff, get_course, get_available_staff, get_all_courses
...
def allocateStaff(course_id, staff_id):
    allocate = allocate_staff(course_id, staff_id)
    allocations = get_allocations()
    staff = get_available_staff(course_id)
    courses = get_all_courses()
    return url_for ('return_staff', course_id=course_id)
